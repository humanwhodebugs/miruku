...
# Function: Start screen recording with gpu-screen-recorder
start_recording() {
  n=1
  # Find the next available file name
  while [[ -e "$HOME/Videos/Screenrecords/screen-recording-$n.mp4" ]]; do ((n++)); done

  # Launch screen recording with desired settings
  gpu-screen-recorder -w screen -c mp4 -s 1366x768 -f 30 -bm qp -q very_high \
    -k h264 -ac opus -a default_output -cursor yes -keyint 2 -encoder cpu \
    -o "$HOME/Videos/Screenrecords/screen-recording-$n.mp4" &

  echo $! >/tmp/screen_recording.pid # Save process ID
  notify-send -t 2000 "Screen Recorder" "Recording started: screen-recording-$n.mp4"
}

# Function: Stop the current screen recording
stop_recording() {
